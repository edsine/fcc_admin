...
use AppBundle\Model\Recruitment\CocAppointmentList;
...
class CocAppointmentListService extends ServiceHelper
{
    private $connection;
    ...
    /**
     * @param CocAppointmentList $cocAppointmentList
     * @return bool
     * @throws AppException
     */
    public function addList(CocAppointmentList $cocAppointmentList): bool
    {
        $outcome = false;
        $statement = null;
        ...
        try {
            //now insert
            $query = "INSERT INTO recruitment_coc_appointment_list 
                (
            ...
            $statement->bindValue(':recruitment_id', $this->getValueOrNull($cocAppointmentList->getRecruitmentId()));
            $statement->bindValue(':total_candidates', $this->getValueOrNull($cocAppointmentList->getTotalCandidates()));
            $statement->bindValue(':attachment_file_name', $this->getValueOrNull($cocAppointmentList->getAttachment()->getFileName()));
            $statement->bindValue(':validation_status', $this->getValueOrNull($cocAppointmentList->getValidationStatus()));
            $statement->bindValue(':data_transfer_status', $this->getValueOrNull($cocAppointmentList->getDataTransferStatus()));
            $statement->bindValue(':record_status', $this->getValueOrNull($cocAppointmentList->getRecordStatus()));
            $statement->bindValue(':created', $this->getValueOrNull($cocAppointmentList->getLastModified()));
            $statement->bindValue(':created_by', $this->getValueOrNull($cocAppointmentList->getLastModifiedByUserId()));
            $statement->bindValue(':last_mod', $this->getValueOrNull($cocAppointmentList->getLastModified()));
            $statement->bindValue(':last_mod_by', $this->getValueOrNull($cocAppointmentList->getLastModifiedByUserId()));
            $statement->bindValue(':selector', $this->getValueOrNull($cocAppointmentList->getSelector()));
    ...
    public function getListsForRecruitment($recruitmentId)
    {
        ...
        try {
            $query = "SELECT d.id,d.recruitment_id,d.total_candidates,d.attachment_file_name
                ...
                FROM recruitment_coc_appointment_list d 
            ...
            if ($record) {
                $cocAppointmentList = new CocAppointmentList();
                $cocAppointmentList->setId($record['id']);
                $cocAppointmentList->setRecruitmentId($record['recruitment_id']);
                $cocAppointmentList->setTotalCandidates($record['total_candidates']);
                $cocAppointmentList->setValidationStatus($record['validation_status']);
                $cocAppointmentList->setDateLastValidated($record['date_last_validated_formatted']);
                $cocAppointmentList->setDataTransferStatus($record['date_data_transfered_formatted']);
                $cocAppointmentList->setRecordStatus($record['record_status']);
                $cocAppointmentList->setCreated($record['created_formatted']);

                $cocAppointmentList->setSelector($record['selector']);

                $recruitmentLongLists[] = $cocAppointmentList;
            }
    ...
    public function getValidationStatus($listId) //return string
    {
        ...
        try {
            ...
            $query = "SELECT validation_status FROM recruitment_coc_appointment_list WHERE id=:id";
            $statement = $this->connection->prepare($query);
            ...
            if ($validationStatus == AppConstants::FATAL_ERROR) {
                try {
                    ...
                    $query = "INSERT INTO recruitment_coc_appointment_list_with_fatal_error
                              (
                                  SELECT l.* 
                                  FROM recruitment_coc_appointment_list l
                                  WHERE l.id=:id
                              )";
                    $statement = $this->connection->prepare($query);
                    $statement->bindValue(':id', $listId);
                    ...
                    $query = "DELETE FROM recruitment_coc_appointment_list 
                              WHERE id=:id";
                    $statement = $this->connection->prepare($query);
                    $statement->bindValue(':id', $listId);
                    $statement->execute();
    ...
    public function updateValidationStatus($listId, $validationStatus): bool
    {
        ...
        try {
            ...
            $query = "UPDATE recruitment_coc_appointment_list 
    ...
    public function transferFromStagingArea($listId, $userProfileId): bool
    {
        ...
        try {
            ...
            if ($listId) {
                ...
                $query = "insert into recruitment_coc_appointment_list_entries 
                            (
                             id, recruitment_coc_appointment_list_id, serial_no, surname, first_name, other_names, date_of_birth
                            ...
                            ( 
                             select 
                             s.id, s.recruitment_coc_appointment_list_id, s.serial_no, s.surname, s.first_name, s.other_names, s.date_of_birth
                             ...
                             from recruitment_coc_appointment_list_entries_staging s 
                             where s.recruitment_coc_appointment_list_id = $listId
                            )";
                $statement = $this->connection->prepare($query);
                $statement->execute();
    ...
    public function getDataTransferStatus($listId)
    {
        ...
        try {
            ...
            $query = "SELECT data_transfer_status FROM recruitment_coc_appointment_list WHERE id=:id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':id', $listId);
            $statement->execute();
     ...
     * @throws \Doctrine\DBAL\ConnectionException
     */
    public function deleteList(CocAppointmentList $cocAppointmentList): bool
    {
        $outcome = false;
        $statement = null;
        ...
        try {
            ...
            $query = "SELECT attachment_file_name FROM recruitment_coc_appointment_list WHERE recruitment_id = :recruitment_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_id', $cocAppointmentList->getRecruitmentId());
            $statement->execute();
            ...
            $query = "DELETE FROM recruitment_coc_appointment_list_entries WHERE recruitment_coc_appointment_list_id = :recruitment_coc_appointment_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_coc_appointment_list_id', $cocAppointmentList->getId());
            $statement->execute();

            //delete the long list staging
            $query = "DELETE FROM recruitment_coc_appointment_list_entries_staging WHERE recruitment_coc_appointment_list_id = :recruitment_coc_appointment_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_coc_appointment_list_id', $cocAppointmentList->getId());
            $statement->execute();

            //delete failed validation
            $query = "DELETE FROM recruitment_coc_appointment_list_failed_validations WHERE recruitment_coc_appointment_list_id = :recruitment_coc_appointment_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_coc_appointment_list_id', $cocAppointmentList->getId());
            $statement->execute();

            //delete failed validation detail
            $query = "DELETE FROM recruitment_coc_appointment_list_failed_validations_detail WHERE recruitment_coc_appointment_list_id = :recruitment_coc_appointment_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_coc_appointment_list_id', $cocAppointmentList->getId());
            $statement->execute();

            //delete the main list itself
            $query = "DELETE FROM recruitment_coc_appointment_list WHERE id = :id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':id', $cocAppointmentList->getId());
            $statement->execute();
    ...
    public function searchCandidates(CandidateSearchCriteria $searchCriteria, Paginator $paginator, $pageDirection): array
    {
        ...
        try {
            ...
            $where = array("d.recruitment_coc_appointment_list_id=$searchListId");
            ...
            $countQuery = "SELECT COUNT(d.id) AS totalRows 
                          FROM recruitment_coc_appointment_list_entries d 
                          JOIN recruitment_coc_appointment_list l on d.recruitment_coc_appointment_list_id = l.id
            ...
            $query = "SELECT d.id,d.recruitment_coc_appointment_list_id
    ...
    public function searchFailedValidationDetail($listId, Paginator $paginator, $pageDirection): array
    {
        ...
        try {
            ...
            //fetch total matching rows
            $countQuery = "SELECT COUNT(d.id) AS totalRows 
                FROM recruitment_coc_appointment_list_failed_validations_detail d 
                WHERE d.recruitment_coc_appointment_list_id=:recruitment_coc_appointment_list_id ";
            $statement = $this->connection->prepare($countQuery);
            $statement->bindValue(':recruitment_coc_appointment_list_id', $listId);
            ...
            //now exec SELECT with limit clause
            $query = "SELECT d.id,d.recruitment_coc_appointment_list_id,d.failed_surname,d.failed_first_name,d.failed_other_name,d.failed_date_of_birth,d.failed_state_of_origin
                ...
                FROM recruitment_coc_appointment_list_failed_validations_detail d 
                JOIN recruitment_coc_appointment_list_entries_staging s on (d.id = s.id and d.recruitment_coc_appointment_list_id = s.recruitment_coc_appointment_list_id)
                WHERE d.recruitment_coc_appointment_list_id=:recruitment_coc_appointment_list_id 
