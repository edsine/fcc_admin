...
class LongListTaskService extends ServiceHelper
{
    private $connection;
     ...
     */
    public function importFromCSV($mysqlLoadCSVPath, LongList $longList): int
    {
        $affectedRows = 0;
        $statement = null;
        ...
        try {
            ...
            $query = "LOAD DATA INFILE '$mysqlLoadCSVPath'
                    INTO TABLE recruitment_long_list_candidates_staging 
                    FIELDS TERMINATED BY ',' 
                    OPTIONALLY ENCLOSED BY '\"' 
                    LINES TERMINATED BY '\n' 
                    (long_list_id, serial_no, surname, first_name, other_names, date_of_birth
            ...
            $query = "SELECT count(*) FROM recruitment_long_list_candidates_staging WHERE long_list_id=:long_list_id";
            ...
            $query = "UPDATE recruitment_long_list SET total_candidates=:total_candidates "
                . " WHERE id=:id";
            $statement = $this->connection->prepare($query);
     ...
     * @throws \Doctrine\DBAL\ConnectionException
     */
    public function importMultiInsert($candidates, LongList $longList): int
    {
        $affectedRows = 0;
        $statement = null;
        ...
        try {
            ...
            foreach ($candidates as $candidate) {
                ...
                $candidateDataRecord['long_list_id'] = $this->getValueOrNull($candidate[0]);
            ...
            $outcome = $this->pdoMultiInsertReturnTotal('recruitment_long_list_candidates_staging', $batchCandidateData);
