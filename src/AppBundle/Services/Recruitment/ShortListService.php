...
use AppBundle\Model\Recruitment\ShortList;
...
class ShortListService extends ServiceHelper
{
    private $connection;
     ...
     * @throws AppException
     */
    public function addList(ShortList $shortList): bool
    {
        $outcome = false;
        $statement = null;
        ...
        try {
            //now insert
            $query = "INSERT INTO recruitment_short_list 
                (
            ...
            $statement->bindValue(':recruitment_id', $this->getValueOrNull($shortList->getRecruitmentId()));
            $statement->bindValue(':total_candidates', $this->getValueOrNull($shortList->getTotalCandidates()));
            $statement->bindValue(':attachment_file_name', $this->getValueOrNull($shortList->getAttachment()->getFileName()));
            $statement->bindValue(':validation_status', $this->getValueOrNull($shortList->getValidationStatus()));
            $statement->bindValue(':data_transfer_status', $this->getValueOrNull($shortList->getDataTransferStatus()));
            $statement->bindValue(':record_status', $this->getValueOrNull($shortList->getRecordStatus()));
            $statement->bindValue(':created', $this->getValueOrNull($shortList->getLastModified()));
            $statement->bindValue(':created_by', $this->getValueOrNull($shortList->getLastModifiedByUserId()));
            $statement->bindValue(':last_mod', $this->getValueOrNull($shortList->getLastModified()));
            $statement->bindValue(':last_mod_by', $this->getValueOrNull($shortList->getLastModifiedByUserId()));
            $statement->bindValue(':selector', $this->getValueOrNull($shortList->getSelector()));
    ...
    public function getListsForRecruitment($recruitmentId)
    {
        ...
        try {
            ...
            if ($record) {
                ...
                $shortList->setId($record['id']);
                $shortList->setRecruitmentId($record['recruitment_id']);
                $shortList->setTotalCandidates($record['total_candidates']);
                $shortList->setValidationStatus($record['validation_status']);
                $shortList->setDateLastValidated($record['date_last_validated_formatted']);
                $shortList->setDataTransferStatus($record['date_data_transfered_formatted']);
                $shortList->setRecordStatus($record['record_status']);
                $shortList->setCreated($record['created_formatted']);

                $shortList->setSelector($record['selector']);

                $recruitmentShortLists[] = $shortList;
            }
    ...
    public function getValidationStatus($listId) //return string
    {
        ...
        try {
            ...
            $query = "SELECT validation_status FROM recruitment_short_list WHERE id=:id";
            $statement = $this->connection->prepare($query);
            ...
            if ($validationStatus == AppConstants::FATAL_ERROR) {
                try {
                    ...
                    $query = "INSERT INTO recruitment_short_list_with_fatal_error
                              (
                                  SELECT l.* 
                                  FROM recruitment_short_list l
                                  WHERE l.id=:id
                              )";
                    $statement = $this->connection->prepare($query);
                    $statement->bindValue(':id', $listId);
                    ...
                    $query = "DELETE FROM recruitment_short_list 
                              WHERE id=:id";
                    $statement = $this->connection->prepare($query);
                    $statement->bindValue(':id', $listId);
                    $statement->execute();
    ...
    public function updateValidationStatus($listId, $validationStatus): bool
    {
        ...
        try {
            ...
            $query = "UPDATE recruitment_short_list 
    ...
    public function transferFromStagingArea($listId, $userProfileId): bool
    {
        ...
        try {
            ...
            if ($listId) {
                ...
                $query = "insert into recruitment_short_list_candidates 
                            (
                             id, short_list_id, serial_no, surname, first_name, other_names, date_of_birth
                            ...
                            ( 
                             select 
                             s.id, s.short_list_id, s.serial_no, s.surname, s.first_name, s.other_names, s.date_of_birth
                             ...
                             from recruitment_short_list_candidates_staging s 
                             where s.short_list_id = $listId
                            )";
                $statement = $this->connection->prepare($query);
                $statement->execute();
    ...
    public function getDataTransferStatus($listId)
    {
        ...
        try {
            ...
            $query = "SELECT data_transfer_status FROM recruitment_short_list WHERE id=:id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':id', $listId);
            $statement->execute();
     ...
     * @throws \Doctrine\DBAL\ConnectionException
     */
    public function deleteList(ShortList $shortList): bool
    {
        $outcome = false;
        $statement = null;
        ...
        try {
            ...
            $query = "SELECT attachment_file_name FROM recruitment_short_list WHERE recruitment_id = :recruitment_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_id', $shortList->getRecruitmentId());
            $statement->execute();
            ...
            $query = "DELETE FROM recruitment_short_list_candidates WHERE short_list_id = :short_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':short_list_id', $shortList->getId());
            $statement->execute();

            //delete the long list staging
            $query = "DELETE FROM recruitment_short_list_candidates_staging WHERE short_list_id = :short_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':short_list_id', $shortList->getId());
            $statement->execute();

            //delete failed validation
            $query = "DELETE FROM recruitment_short_list_failed_validations WHERE short_list_id = :short_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':short_list_id', $shortList->getId());
            $statement->execute();

            //delete failed validation detail
            $query = "DELETE FROM recruitment_short_list_failed_validations_detail WHERE short_list_id = :short_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':short_list_id', $shortList->getId());
            $statement->execute();

            //delete the main list itself
            $query = "DELETE FROM recruitment_short_list WHERE id = :id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':id', $shortList->getId());
            $statement->execute();
    ...
    public function searchCandidates(CandidateSearchCriteria $searchCriteria, Paginator $paginator, $pageDirection): array
    {
        ...
        try {
            ...
            $where = array("d.short_list_id=$searchListId");
            ...
            $countQuery = "SELECT COUNT(d.id) AS totalRows 
                          ...
                          JOIN recruitment_short_list l on d.short_list_id = l.id
            ...
            $query = "SELECT d.id,d.short_list_id
    ...
    public function searchFailedValidationDetail($listId, Paginator $paginator, $pageDirection): array
    {
        ...
        try {
            ...
            //fetch total matching rows
            $countQuery = "SELECT COUNT(d.id) AS totalRows 
                FROM recruitment_short_list_failed_validations_detail d 
                WHERE d.short_list_id=:short_list_id ";
            $statement = $this->connection->prepare($countQuery);
            $statement->bindValue(':short_list_id', $listId);
            ...
            //now exec SELECT with limit clause
            $query = "SELECT d.id,d.short_list_id,d.failed_surname,d.failed_first_name,d.failed_other_name,d.failed_date_of_birth,d.failed_state_of_origin
                ...
                FROM recruitment_short_list_failed_validations_detail d 
                JOIN recruitment_short_list_candidates_staging s on (d.id = s.id and d.short_list_id = s.short_list_id)
                WHERE d.short_list_id=:short_list_id 
