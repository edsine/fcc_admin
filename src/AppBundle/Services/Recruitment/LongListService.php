...
use AppBundle\Model\Recruitment\LongList;
...
class LongListService extends ServiceHelper
{
    private $connection;
     ...
     * @throws AppException
     */
    public function addList(LongList $longList): bool
    {
        $outcome = false;
        $statement = null;
        ...
        try {
            //now insert
            $query = "INSERT INTO recruitment_long_list 
                (
            ...
            $statement->bindValue(':recruitment_id', $this->getValueOrNull($longList->getRecruitmentId()));
            $statement->bindValue(':total_candidates', $this->getValueOrNull($longList->getTotalCandidates()));
            $statement->bindValue(':attachment_file_name', $this->getValueOrNull($longList->getAttachment()->getFileName()));
            $statement->bindValue(':validation_status', $this->getValueOrNull($longList->getValidationStatus()));
            $statement->bindValue(':data_transfer_status', $this->getValueOrNull($longList->getDataTransferStatus()));
            $statement->bindValue(':record_status', $this->getValueOrNull($longList->getRecordStatus()));
            $statement->bindValue(':created', $this->getValueOrNull($longList->getLastModified()));
            $statement->bindValue(':created_by', $this->getValueOrNull($longList->getLastModifiedByUserId()));
            $statement->bindValue(':last_mod', $this->getValueOrNull($longList->getLastModified()));
            $statement->bindValue(':last_mod_by', $this->getValueOrNull($longList->getLastModifiedByUserId()));
            $statement->bindValue(':selector', $this->getValueOrNull($longList->getSelector()));
    ...
    public function getListsForRecruitment($recruitmentId)
    {
        ...
        try {
            ...
            if ($record) {
                ...
                $longList->setId($record['id']);
                $longList->setRecruitmentId($record['recruitment_id']);
                $longList->setTotalCandidates($record['total_candidates']);
                $longList->setValidationStatus($record['validation_status']);
                $longList->setDateLastValidated($record['date_last_validated_formatted']);
                $longList->setDataTransferStatus($record['date_data_transfered_formatted']);
                $longList->setRecordStatus($record['record_status']);
                $longList->setCreated($record['created_formatted']);

                $longList->setSelector($record['selector']);

                $recruitmentLongLists[] = $longList;
            }
    ...
    public function getValidationStatus($listId) //return string
    {
        ...
        try {
            ...
            $query = "SELECT validation_status FROM recruitment_long_list WHERE id=:id";
            $statement = $this->connection->prepare($query);
            ...
            if ($validationStatus == AppConstants::FATAL_ERROR) {
                try {
                    ...
                    $query = "INSERT INTO recruitment_long_list_with_fatal_error
                              (
                                  SELECT l.* 
                                  FROM recruitment_long_list l
                                  WHERE l.id=:id
                              )";
                    $statement = $this->connection->prepare($query);
                    $statement->bindValue(':id', $listId);
                    ...
                    $query = "DELETE FROM recruitment_long_list 
                              WHERE id=:id";
                    $statement = $this->connection->prepare($query);
                    $statement->bindValue(':id', $listId);
                    $statement->execute();
    ...
    public function updateValidationStatus($listId, $validationStatus): bool
    {
        ...
        try {
            ...
            $query = "UPDATE recruitment_long_list 
    ...
    public function transferFromStagingArea($listId, $userProfileId): bool
    {
        ...
        try {
            ...
            if ($listId) {
                ...
                $query = "insert into recruitment_long_list_candidates 
                            (
                             id, long_list_id, serial_no, surname, first_name, other_names, date_of_birth
                            ...
                            ( 
                             select 
                             s.id, s.long_list_id, s.serial_no, s.surname, s.first_name, s.other_names, s.date_of_birth
                             ...
                             from recruitment_long_list_candidates_staging s 
                             where s.long_list_id = $listId
                            )";
                $statement = $this->connection->prepare($query);
                $statement->execute();
    ...
    public function getDataTransferStatus($listId)
    {
        ...
        try {
            ...
            $query = "SELECT data_transfer_status FROM recruitment_long_list WHERE id=:id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':id', $listId);
            $statement->execute();
     ...
     * @throws \Doctrine\DBAL\ConnectionException
     */
    public function deleteList(LongList $longList): bool
    {
        $outcome = false;
        $statement = null;
        ...
        try {
            ...
            $query = "SELECT attachment_file_name FROM recruitment_long_list WHERE recruitment_id = :recruitment_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_id', $longList->getRecruitmentId());
            $statement->execute();
            ...
            $query = "DELETE FROM recruitment_long_list_candidates WHERE long_list_id = :long_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':long_list_id', $longList->getId());
            $statement->execute();

            //delete the long list staging
            $query = "DELETE FROM recruitment_long_list_candidates_staging WHERE long_list_id = :long_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':long_list_id', $longList->getId());
            $statement->execute();

            //delete failed validation
            $query = "DELETE FROM recruitment_long_list_failed_validations WHERE long_list_id = :long_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':long_list_id', $longList->getId());
            $statement->execute();

            //delete failed validation detail
            $query = "DELETE FROM recruitment_long_list_failed_validations_detail WHERE long_list_id = :long_list_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':long_list_id', $longList->getId());
            $statement->execute();

            //delete the main list itself
            $query = "DELETE FROM recruitment_long_list WHERE id = :id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':id', $longList->getId());
            $statement->execute();
    ...
    public function searchCandidates(CandidateSearchCriteria $searchCriteria, Paginator $paginator, $pageDirection): array
    {
        ...
        try {
            ...
            $where = array("d.long_list_id=$searchListId");
            ...
            $countQuery = "SELECT COUNT(d.id) AS totalRows 
                          FROM recruitment_long_list_candidates d 
                          JOIN recruitment_long_list l on d.long_list_id = l.id
            ...
            $query = "SELECT d.id,d.long_list_id
    ...
    public function searchFailedValidationDetail($listId, Paginator $paginator, $pageDirection): array
    {
        ...
        try {
            ...
            //fetch total matching rows
            $countQuery = "SELECT COUNT(d.id) AS totalRows 
                FROM recruitment_long_list_failed_validations_detail d 
                WHERE d.long_list_id=:long_list_id ";
            $statement = $this->connection->prepare($countQuery);
            $statement->bindValue(':long_list_id', $listId);
            ...
            //now exec SELECT with limit clause
            $query = "SELECT d.id,d.long_list_id,d.failed_surname,d.failed_first_name,d.failed_other_name,d.failed_date_of_birth,d.failed_state_of_origin
                ...
                FROM recruitment_long_list_failed_validations_detail d 
                JOIN recruitment_long_list_candidates_staging s on (d.id = s.id and d.long_list_id = s.long_list_id)
                WHERE d.long_list_id=:long_list_id 
