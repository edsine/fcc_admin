...
use AppBundle\Model\Recruitment\LongListCandidate;
...
class LongListCandidatesService extends ServiceHelper
{
    private $connection;
        ...
        try {
            ...
            if ($records) {
                ...
                for ($i = 0; $i < count($records); $i++) {
                    ...
                    $candidate = new LongListCandidate();
    ...
    public function getLongListCandidate($selector):?LongListCandidate
    {
        $candidate = null;
        $statement = null;
    ...
    public function importLongListMultiInsert($longListCandidates, Recruitment $recruitment): int
    {
        $affectedRows = 0;
        $statement = null;
        ...
        try {
            ...
            foreach ($longListCandidates as $candidate) {
            ...
            $outcome = $this->pdoMultiInsertReturnTotal('recruitment_long_list_candidates', $batchCandidateData);

            $query = "SELECT count(*) FROM recruitment_long_list_candidates WHERE recruitment_id=:recruitment_id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':recruitment_id', $recruitment->getId());
            $statement->execute();
            ...
            $query = "UPDATE 
                    recruitment 
                    SET long_list_upload_file_name=:long_list_upload_file_name 
                    , long_list_last_uploaded=:long_list_last_uploaded 
                    , total_long_list_candidates=:total_long_list_candidates 
                    WHERE selector =:selector";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':long_list_upload_file_name', $this->getValueOrNull($recruitment->getLongListAttachment()->getFileName()));
            $statement->bindValue(':long_list_last_uploaded', $this->getValueOrNull($recruitment->getLongListLastUploaded()));
            $statement->bindValue(':total_long_list_candidates', $affectedRows);
    ...
    public function deleteLongList(Recruitment $recruitment): bool
    {
        $outcome = false;
        $statement = null;
        ...
        try {
            ...
            $query = "select short_list_upload_file_name from recruitment where id = :id";
            $statement = $this->connection->prepare($query);
            $statement->bindValue(':id', $recruitment->getId());
            $statement->execute();
            ...
            $query = "UPDATE recruitment 
                SET 
                long_list_upload_file_name= NULL
                ,long_list_last_uploaded= NULL
                ,total_long_list_candidates= NULL
                ,last_mod=:last_mod
